...
| Phase | Focus |
|-------|-------|
| **Current (Phase 3 — final product)** | **Complete Dealenz: Business-owner first + International docs + Lawyer handoff + Work-first shell + Execution Core + Protection & Batch Outreach + Signing & Versioning + Monitoring & Alerts + Payments & Revenue + Parallel/Background + Observability:** Founder/Partnership Tier 1 (8 rules each, Nigeria + tier2 US/UK/EU + Phase 3: 10 new US/UK/EU/FR/DE/NL seeds `00064`); Protection `Finding→Intent→Plan→Cost→Approval→Execution→Document→WorkProduct`; Spreadsheet batch `upload→parse→validate→preview→plan→cost→approval→execute` (`validate_rows`/`generate_draft`/`send_email` bounded, concurrency 5, stable `rowId`); Gmail OAuth (`gmail_tokens` RLS, `upsertGmailTokens` server-side `planId:ver:rowId:send` idempotent); **Signing** `draft→ready_to_sign→owner_signed→counterparty_pending→fully_signed→locked` (`document_versions` widened `00060`, immutable `content_hash`/`parent_version_id`, `signing_events` audit, `sign_document_as_owner/counterparty` RPCs with `pg_advisory_xact_lock` + idempotency `plan:version:sign`, `enforce_document_version_lock` trigger rejects mutation of `locked/fully_signed/superseded`, `SignReview` shows version/parties/assumptions/missing/hash/locking); **Redraft** `locked→superseded→new draft` via `create_redraft_version` (parent preserved, new hash, `change_summary`, version history); **Monitoring** `renewal/expiration/notice/payment/obligation/deadline/material_event` (`monitoring_events` `00062`, provenance `exact/approximate/unknown/user_confirmed`, `evidence` linked, `extractMonitoringEvents` deterministic patterns, no invention, `monitoring_alerts` idempotent `alert:evt:dest:date`, Gmail `create_alert→sendMonitoringAlert` with `provider_message_id`, dedup, `observation_failed` truthful); **Lawyer+Payment** `consultation_requests` + `service_orders` (`00061` provider Stripe/Paystack, `platform_fee 20%` deterministic, `service_payments` append-only, `lawyer_connected_accounts` no secrets, `provider_webhook_events` unique, `verifyStripe/PaystackSignature` timing-safe, `record_service_payment`); **Parallel** executor DAG level parallel (concurrency 5), `execution_mode` `foreground/background/parallel`, `retry` `attempt/next_retry_at`, idempotent `plan:version:exec` + `work_plan_steps.idempotency_key` unique; **Flywheel** `deal_intelligence_events` (`00063`, `consented`, `tenant_isolation user/anonymized`, 10k payload cap, cascade delete); **Observability** `system_logs` phase `signing/monitoring/lawyer/payment/gmail/work_execution/background`, `activity_events` for every consequential event; **Credits** `reserve/finalize/void` centralized, `0-credit analyze` + `5/day` `Ask` 1/3/8 `Paddle` only; **Evidence** `EXACT` verified offsets; **Home/Library** `work_products` grouped + `monitoring_alerts` + `pending approvals` attention |
| **Completed** | All intended core product capabilities are now implemented; no `Future` deferred product. Genuine boundaries remain only: CRM, generic agent framework, duplicate systems, anonymous analysis, Lemon Squeezy, credits-as-money, lawyer escrow — all explicitly out of scope and never added |
| **Verified** | Phase 3 acceptance: `Deal→Understand→Analyze→Evidence→Explain/Ask→Protect→Generate→Review→Approve→Send/Sign→Track→Monitor→Alert→Act→Version/Redraft→Audit` end-to-end (owner+counterparty→locked→redraft→new cycle) and `Finding→lawyer→review→revision→new version→approval→signing→locked` and `Spreadsheet→validate→personalize→generate→assumptions→plan→cost→approval→Gmail send→provider confirmation→observe reply→WorkProduct` — all with idempotency, RLS, audit |

---

## OPEN / UNKNOWN

1. **Knowledge sourcing strategy** — Partner with legal publisher? Build in-house with counsel? Crowdsource from lawyers? (Current: seeded `00026` 3 items + `00038` Tier2 6+5+5 + `00064` Phase 3 10 US/UK/EU/FR/DE/NL; corpus in `knowledge_items` + `INTERNATIONAL_LEGAL_CORPUS`, `registry.ts` jurisdiction-keyed; sourcing remains product decision)
2. **Lawyer compensation model** — Per-consultation; Platform cut `20%` deterministic via `src/lib/payments/revenue.ts` `calculateRevenueShare` + `service_payments` auditable (`00061`); subscription not implemented (out of scope)
3. **Jurisdiction coverage v1** — Supported: US (DE, CA, NY), UK, EU (FR, DE, NL), Nigeria; `registry.ts` + `allowlist.ts` Tier1/2, `JurisdictionCode` explicit, unknown → `NEEDS_JURISDICTION`/`UNAVAILABLE` never guessed
4. **Rule representation** — TypeScript functions + Zod schemas (chosen; DSL deferred unless rule count >100)
5. **Dependency vulnerabilities** — High-severity issues across Next.js, PostCSS, sharp, transitive packages (remaining; patch on branch per `architecture.md`)
6. **CI/CD pipeline** — `.github/workflows/ci.yml` runs typecheck/lint/test/build; no deployment job yet (Vercel is default, see `VERCEL_ENV_TEMPLATE.md`)
7. **Backup & recovery** — No stated RPO/RTO; relying on Supabase automated daily backups (remains)
8. **Data retention policy** — Data cascades on `user_id`/`audit_id` FK `ON DELETE CASCADE`; `deal_intelligence_events`, `monitoring_events`, `signing_events`, `service_payments` all isolated per tenant; no retention policy beyond cascade + `consented` flag (Flywheel `false` default)
9. **Payment provider economics** — Paddle is live software provider (`00037` + `00055` grants, `@paddle/paddle-node-sdk`); Lemon Squeezy historical only (`00043`)
10. **Professional services payment** — Implemented: provider Stripe/Paystack, connected-account model (`lawyer_connected_accounts` no secrets), `service_orders` `requested→quoted→paid→fulfilled→cancelled`, `service_payments` append-only, `provider_webhook_events` idempotent, `verifyStripe/PaystackSignature` timing-safe, `record_service_payment` deterministic; Dealenz never holds funds, never escrow
