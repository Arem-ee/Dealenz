...
- **One shell everywhere:** `src/components/app-shell.tsx` — slim top navbar (logo, thread search, plain credit balance, notifications entry, single account menu), sidebar on desktop, bottom tabs on mobile — wraps `/dashboard`, `/library`, `/chat`, `/settings`, `/document`, `/review` via per-area layouts. Previously only `/dashboard/*` had chrome; sibling screens rendered chromeless with no way back.
- **Home is intentionally single-column and composer-first:** heading, composer, one understated Library link, then recent activity (title + relative time). No action cards, tours, or tooltips — the classifier routes whatever is typed. The split-pane stays where two-pane content exists (thread, library, review). This divergence is deliberate, not a gap.
- **Threads live on Home** (recent activity, all widths) **and in navbar search** (bounded client-side filter over the shell-loaded list). The sidebar holds only Home/Library; the account menu lives only in the navbar.
- **Composer is viewport-pinned by exact heights, not position:fixed:** containers subtract the shell chrome above them (navbar 3.5rem; mobile tab bar 4rem via main padding; thread back bar 3rem), the message list scrolls independently, and the composer sits shrink-0 at the column end. `position:fixed` is avoided so the composer never underlaps the sidebar or the mobile tab bar.
