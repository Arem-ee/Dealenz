...
- **One shell everywhere:** `src/components/app-shell.tsx` — slim top navbar (logo, mobile menu drawer, thread search, plain credit balance, notifications entry, single account menu), hover-expand icon-rail sidebar on desktop (no manual collapse affordance) with a recent-deals list — wraps `/dashboard`, `/library`, `/chat`, `/settings`, `/document`, `/review` via per-area layouts. Previously only `/dashboard/*` had chrome; sibling screens rendered chromeless with no way back. Mobile uses a top-anchored drawer, not a bottom tab bar (web-first: no bottom-app chrome competing with the composer).
- **Home is a deal overview on every width:** portfolio health, deadlines, and the deals table up front; creation through New deal and the capability strip. No action cards, tours, or tooltips — the classifier routes whatever is typed. The split-pane stays where two-pane content exists (thread, library, review). This divergence is deliberate, not a gap.
- **Threads live on Home** (attention queues — All / Needs you / Waiting on others / Signed & guarded — plus recent activity, all widths) **and in navbar search** (bounded client-side filter over the shell-loaded list) **and in the sidebar** (recent 5, desktop hover rail). The sidebar holds only Home/Library; the account menu lives only in the navbar.
- **Library reads as a library:** its own sub-sidebar on desktop (Search deals / From inbox / Rules), compact top tabs on mobile — never floating pills over content.
- **Composer is viewport-pinned by exact heights, not position:fixed:** containers subtract the shell chrome above them (navbar 3.5rem; thread back bar 3rem), the message list scrolls independently, and the composer sits shrink-0 at the column end. `position:fixed` is avoided so the composer never underlaps the sidebar.
...
**Surface (Phase 22C, OpenRouter live):** authenticated Deal Intelligence resolves via centralized `resolveSurfaceConfig("authenticated")` (`src/lib/ai/providers.ts`). Production routes `AUTH_AI_PROVIDER=openai_compatible` at `AI_BASE_URL=https://openrouter.ai/api/v1`, so `AUTH_AI_MODEL` must be an exact OpenRouter `provider/model` id (verified live 2026-09-19, e.g. `anthropic/claude-sonnet-5`); a retired id fails every authenticated call with HTTP 404 (`invalid_request`, no fallback on this path). The Anthropic adapter + Opus fallback engage only when `AUTH_AI_PROVIDER=anthropic`. Domain functions (`extract`, `risk-analysis`, `negotiation`, `generate`) take an explicit surface parameter defaulting to authenticated. There is no anonymous surface.
...
Conflicting material terms used to collapse into a single extracted string at `src/lib/ai/extract.ts:15` before `src/lib/verticals/*/facts.ts` exposed values to rules. Now resolved structurally: the prompts (`src/lib/ai/prompts.ts`) require `;`-separated competing terms, `splitTerms` preserves them as `budgetTerms`/`timelineTerms`, every vertical derives `conflictingPaymentTerms`/`conflictingTimelineTerms` flags with evidence, and consistency rules fire findings on them (covered by `src/lib/ai/extract-structural.test.ts` and `src/lib/verticals/conflicting-terms.test.ts`). Residual limit: preservation leans on the model emitting the `;` separation; heuristic splitting catches common failures. Do not regress this by reintroducing single-string-only consumers — read `(extracted.budgetTerms ?? [])`, never assume a scalar tells the whole story.
...
- **AI Provider Layer**: Provider-agnostic interface (`callAI`) with Gemini, OpenAI-compatible, and Anthropic Claude adapters; authenticated surface routed by `AUTH_AI_*` (OpenRouter live); no anonymous surface
