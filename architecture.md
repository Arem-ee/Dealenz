...
Current implementation provides versioning (`document_versions`, `00012`, `00048`), share tokens and signatures (`share_tokens`, `document_signatures`, `document_signers`, `00013`, `00044`), and the Server Actions above, but the full owner-first → counterparty → locked → new-version lifecycle is not yet complete in UX. Documented as requirement + architectural direction, not claimed as finished.

---

## Signed-Deal Monitoring

Once a deal is fully signed and locked, Dealenz monitors for important events:
...
It should alert the user **before** those events matter. **Email is a required delivery mechanism.**

Part of the complete product — monitoring for renewal/deadline/material events with email alerts, evidence and audit trail, and human gates for consequential actions. Current primitives (`system_logs`, `activity_events`, `document_versions.created_at`, `audits.updated_at`) are not yet a monitoring system; cron/queue/email-sending monitor to be built. Keep it bounded to deal lifecycle, not a generic monitoring platform.
