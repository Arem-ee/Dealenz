...
- **Minimal international credit purchase (Phase 29/31)** — `src/lib/billing/catalog.ts` provider-independent packages (starter 50 / standard 150 / pro 400 credits; USD/GBP/EUR minor units; no NGN), `src/lib/billing/provider.ts` Lemon Squeezy adapter isolated behind `ProviderAdapter` (buy links from env variant ids, mock when unconfigured), server-authoritative `POST /api/billing/checkout` (auth, catalog lookup, throttle, safe return URLs), verified `POST /api/billing/webhook` (HMAC signature, variant→package resolution, amount floor/currency match, non-succeeded never credits, refunds observed without mutation), idempotent `credit_purchases` (`00037`, unique provider+tx; `00043` admits `lemonsqueezy`, Stripe rows preserved as history) + ledger `purchase:<tx>` idempotency key, allocation via existing `credit_ledger` grant (no second ledger), billing UI balance + packages + purchase history + processing-state honesty
