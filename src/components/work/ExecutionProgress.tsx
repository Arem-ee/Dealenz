...
export function ExecutionProgress({ execution, steps }: { execution: WorkExecutionRow | null; steps: PlanStepRow[] }) {
  ...
  return (
    <div className="rounded-xl border bg-card p-5">
      ...
      <div className="mt-1 text-xs text-muted-foreground">Plan v{execution.plan_version} · Started {execution.started_at ? new Date(execution.started_at).toLocaleString() : "—"}</div>
