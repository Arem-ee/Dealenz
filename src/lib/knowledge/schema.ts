...
function checkApplicability(raw: unknown): KnowledgeApplicability {
  ...
  if (raw.dealTypes !== undefined) {
    if (
      ...
      !raw.dealTypes.every((d) => d === "freelance" || d === "generic" || d === "lease" || d === "purchase_sale" || d === "employment" || d === "founder")
    ) {
      throw new Error("Knowledge item has invalid applicability dealTypes")
    }
    out.dealTypes = [...(raw.dealTypes as Array<"freelance" | "generic" | "lease" | "purchase_sale" | "employment" | "founder">)]
  }
