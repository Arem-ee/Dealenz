...
export const EMPLOYMENT_RULES: Rule[] = [
  ...
  rule({
    ruleKey: "employment-conflicting-payment-terms",
    title: "Employment conflicting payment terms",
    description: "Fires when structurally conflicting payment terms are preserved as separate observations.",
    priority: 98,
    category: "consistency",
    scope: { dealTypes: ["employment"] },
    condition: { field: "facts.employment.conflictingPaymentTerms.value", op: "eq", value: true },
    fireOn: true,
    finding: {
  ...
  rule({
    ruleKey: "employment-conflicting-timeline",
    title: "Employment conflicting timeline",
    description: "Fires when structurally conflicting timeline terms are preserved.",
    priority: 97,
    category: "consistency",
    scope: { dealTypes: ["employment"] },
    condition: { field: "facts.employment.conflictingTimelineTerms.value", op: "eq", value: true },
    fireOn: true,
    finding: {
