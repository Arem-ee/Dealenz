...
export const GENERIC_RULES: Rule[] = [
  ...
  rule({
    ...
    category: "consistency",
    scope: { dealTypes: ["generic"] },
    condition: { field: "facts.generic.conflictingPaymentTerms.value", op: "eq", value: true },
    fireOn: true,
    finding: {
      summary: "Conflicting payment terms were found in the provided deal input.",
      severity: "material",
      guidance: "Clarify which payment terms apply: multiple conflicting values were observed as separate observations. Confirm the correct terms in writing.",
    },
    authority: { kind: "product_policy", note: POLICY },
  }),
  rule({
    ...
    description: "Fires when structurally conflicting scope/timeline terms are preserved.",
    priority: 97,
    category: "consistency",
    scope: { dealTypes: ["generic"] },
    condition: { field: "facts.generic.conflictingScope.value", op: "eq", value: true },
    fireOn: true,
    finding: {
      summary: "Conflicting scope terms were found in the provided deal input.",
      severity: "material",
      guidance: "Clarify which scope applies: multiple conflicting terms were observed separately. Confirm the correct scope in writing.",
    },
    authority: { kind: "product_policy", note: POLICY },
  }),
]
