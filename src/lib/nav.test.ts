...
describe("customer navigation IA (single source)", () => {
  it("keeps exactly three primary destinations: Home, Vault, Settings", () => {
  ...
  it("matches nested routes to their entry without drift", () => {
    expect(isActiveEntry("/vault", "/vault")).toBe(true)
    ...
    expect(isActiveEntry("/chat/abc", "/vault")).toBe(false)
  })
