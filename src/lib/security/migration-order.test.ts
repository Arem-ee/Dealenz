...
// standing-instructions head must exist exactly once each, in order, plus
...
describe("migration chain presence and order", () => {
  it("contains 00001 through 00082 exactly once, in order", () => {
    const files = readdirSync(join(process.cwd(), "supabase", "migrations"))
    const numbered = files
      .map((f) => /^(\d{5})_.*\.sql$/.exec(f)?.[1])
      .filter((n): n is string => n !== undefined)
      .sort()
    const expected = Array.from({ length: 82 }, (_, i) => String(i + 1).padStart(5, "0"))
